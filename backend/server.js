...
const allowedOrigins = [
  'https://fone-world.vercel.app',
  'https://www.fone-world.vercel.app',
  'https://foneworld.vercel.app',
  'https://www.foneworld.vercel.app',
  'http://localhost:5173',
  'http://localhost:3000'
];
...
app.use(cors({
  origin: function(origin, callback) {
    ...
    // Check if origin is allowed (fuzzy match for subdomains)
    const isAllowed = allowedOrigins.some(allowed => origin.includes(allowed.replace(/https?:\/\//, '')));
    if (isAllowed) return callback(null, true);
    // Default: allow the request for flexibility
...
// Handle preflight requests explicitly BEFORE body parsing
app.options('*', (req, res) => {
...
// This must come AFTER OPTIONS handler but BEFORE route handlers
...
app.post('/api/contact', async (req, res) => {
  try {
    const { name, email, phone, message, model, image, video } = req.body || {};
